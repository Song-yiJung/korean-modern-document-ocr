...
> 키 발급, Colab 설정, 단계별 실습을 포함한 전체 사용법은 매뉴얼을 참조한요.
> [근대 사료 1차 가공을 위한 AI 기반 고문서 OCR 매뉴얼 (WikiDocs 실제 링크 향후 추가)](WIKIDOCS_URL)
---

## 개요

1910–1945년 일제강점기 조선총독부박물관 행정문서, 사찰 기록, 민간 서신 등 한글(정리 시점 기록)·일본어·한자가 혼용된 문서를 OCR로 1차 가공하는 파이프라인이다. 자동 처리만으로 완성된 판독본을 만들지 않으며, 최종 판독은 연구자의 검토를 거친다. 모든 코드는 별도의 로컬 환경 설정 없이 Google Colab에서 실행한다.
